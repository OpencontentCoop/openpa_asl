{set_defaults( hash(
  'wrapper_class', cond(is_set($block.custom_attributes.wrapper_class), $block.custom_attributes.wrapper_class, 'py-5 position-relative'),
        ...
        <div class="row" id="remote-gui-{$block.id}">
            ...
            <section class="{if $facets|count()}col-12 col-lg-8 {else}col-12{/if} pt-lg-2 pb-lg-2">
                        ...
                        <ul class="nav d-block nav-pills text-right{if or($showGrid|not(), $showMap|not())} hide{/if}">
                            ...
                            {if $showGrid}
                                <li class="nav-item pr-1 pe-1 text-center d-inline-block">
                                    <a data-toggle="tab" data-bs-toggle="tab"
                                       class="nav-link {if $showMap|not} active{/if} rounded view-selector"
                ...
                <div class="items tab-content">
                    {if $showMap}
                        <section id="remote-gui-{$block.id}-geo" class="tab-pane active p-0">
                            <div id="remote-gui-{$block.id}-map" style="width: 100%; height: 700px"></div>
                        </section>
                    {/if}
                    {if $showGrid}
                        <section id="remote-gui-{$block.id}-list" class="tab-pane {if $showMap|not} active{/if} pt-0 pl-0 ps-0"></section>
                    {/if}
                </div>
            </section>
                ...
                <div class="col-12 col-lg-4 ps-lg-5 order-first order-sm-last">
                    <ul class="nav d-block nav-pills text-right{if or($showGrid|not(), $showMap|not())} hide{/if}">
                        ...
                        {if $showGrid}
                            <li class="nav-item pr-1 pe-1 text-center d-inline-block">
                                <a data-toggle="tab" data-bs-toggle="tab"
                                   class="nav-link {if $showMap|not} active{/if} rounded view-selector text-uppercase"
                                   href="#remote-gui-{$block.id}-list">
