...
{if $attribute.object.class_identifier|eq('public_service')}
     ...
     {if and($service_widget_url_info.builtin|eq('inefficiency'), $context|eq('main'))}
          ...
          <a class="mr-2 {$secondary_css_class}" href="{$inefficiency_dataset.main_node.url_alias|ezurl(no)}">{$inefficiency_dataset.name|wash()}</a>
          {/if}
          {undef $inefficiency_dataset}
     {/if}
     {undef $service_widget_url_info}
{else}
     <a class="mr-2 {$secondary_css_class}" href="{$attribute.content|wash( xhtml )}">
