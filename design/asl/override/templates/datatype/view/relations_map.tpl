    ...
    {foreach $attribute.content.relation_list as $relation_item}
        {if $relation_item.in_trash|not()}
            ...
            {if $content_object.can_read}
                ...
                {/if}
                {if $content_object|has_attribute('geo')}
                    {set $markers = $markers|append($content_object|attribute('geo').content)}
                {/if}
            {/if}
...
{if or(count($node_list)|gt(0), $has_geo)}
    ...
    <div class="map-wrapper map-column mt-4 mb-5" style="margin-left: -14.33333%;">
