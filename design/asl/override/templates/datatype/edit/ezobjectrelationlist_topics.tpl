...
{if $class_content.selection_type|ne( 0 )}
        ...
        {if array(1,2,4,6)|contains($class_content.selection_type)}
            ...
            <select class="{$html_class}" name="{$attribute_base}_data_object_relation_list_{$attribute.id}[]" {if $class_content.selection_type|eq(4)}multiple{/if}>
                ...
                {foreach $topics_tree as $topic}
                    <option value="{$topic.contentobject_id}"
                    {if ne( count( $attribute.content.relation_list ), 0)}
                    {foreach $attribute.content.relation_list as $item}
                         {if eq( $item.contentobject_id, $topic.contentobject_id )}
                            selected="selected"
                            {break}
                         {/if}
                    {/foreach}
                    {/if}
                    >{$topic.name|wash}</option>
        ...
        {if array(3,5)|contains($class_content.selection_type)}
            ...
            {foreach $topics_tree as $topic}
                {if and($topic, $custom_topic_container, $topic.contentobject_id|eq($custom_topic_container.id))}{skip}{/if}
            <li class="col-4">
                <div class="form-check">
                  <input id="check-{$attribute.id}-{$topic.contentobject_id}"
                         class="form-check-input"
                         type="checkbox"
                         name="{$attribute_base}_data_object_relation_list_{$attribute.id}[{$topic.node_id}]"
                         value="{$topic.contentobject_id}"
                      {if ne( count( $attribute.content.relation_list ), 0)}
                      {foreach $attribute.content.relation_list as $item}
                           {if eq( $item.contentobject_id, $topic.contentobject_id )}
                                  checked="checked"
                      ...
                      {/if}
                      />
                  <label class="form-check-label" for="check-{$attribute.id}-{$topic.contentobject_id}">{$topic.name|wash}{if $topic.children|count()} <i aria-hidden="true" class="fa fa-plus-square-o"></i>{/if}</label>
                </div>
                {if $topic.children|count()}
                        ...
                        <div class="form-check">
                            <input id="check-{$attribute.id}-{$child.contentobject_id}"
                                   ...
                                   value="{$child.contentobject_id}"
                                    ...
                                    {/if}
                            />
                            <label class="form-check-label" for="check-{$attribute.id}-{$child.contentobject_id}">{$child.name|wash}{if $child.children|count()} <i aria-hidden="true" class="fa fa-plus-square-o"></i>{/if}</label>
                        </div>
                        {if $child.children|count()}
                        <ul class="list-unstyled ml-3 ms-3">
                            {foreach $child.children as $sub_child}
                                ...
                                <div class="form-check">
                                    <input id="check-{$attribute.id}-{$sub_child.contentobject_id}"
                                           ...
                                           value="{$sub_child.contentobject_id}"
                                            ...
                                            {/if}
                                    />
                                    <label class="form-check-label" for="check-{$attribute.id}-{$sub_child.contentobject_id}">{$sub_child.name|wash}{if $sub_child.children|count()} <i aria-hidden="true" class="fa fa-plus-square-o"></i>{/if}</label>
                                </div>
                                {if $sub_child.children|count()}
                                <ul class="list-unstyled ml-3 ms-3">
                                    {foreach $sub_child.children as $sub_sub_child}
                                    <li>
                                        <div class="form-check">
