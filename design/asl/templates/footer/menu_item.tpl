...
{if $show_main_link}
<h3 class="footer-heading-title border-0">
    <a href="{$href}" class="text-decoration-none" title="{'Go to page'|i18n('bootstrapitalia')}: {$menu_item.item.name|wash()}">{$menu_item.item.name|wash()}</a>
</h3>
{/if}
{if $has_children}
