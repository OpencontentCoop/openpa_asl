    ...
    <div class="row">
        ...
        {if openpaini('CreditsSettings', 'UseDearOldCreditsTemplate', 'disabled')|eq('enabled')}
            ...
            {if openpaini('CreditsSettings', 'ShowCredits', 'enabled')|eq('enabled')}
                ...
                {if openpaini('CreditsSettings', 'CodeVersion', false())}<a class="" href="{openpaini('CreditsSettings', 'CodeUrl', false())}">{openpaini('CreditsSettings', 'CodeVersion', false())}</a>{/if}
            {/if}
            &#8212; concept & design by {display_icon('it-designers-italia', 'svg', 'icon icon-xs icon-primary')} <a class="" href="https://designers.italia.it/kit/comuni/"> Designers Italia</a>
        {else}
            ...
            {/if}
        {/if}
            &middot; <a data-login-bottom-button href="{"/user/login"|ezurl(no)}" class="">{'Site editors access'|i18n('bootstrapitalia')}</a>
            {literal}
                <script>
