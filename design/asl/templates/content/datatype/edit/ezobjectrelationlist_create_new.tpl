{if and( is_set( $attribute.class_content.class_constraint_list ), $attribute.class_content.class_constraint_list|count|ne( 0 ), $browse_object_start_node )}
    ...
    <div class="btn-group create-relation-buttons">
        ...
        <button type="button" class="btn btn-sm btn-info ml-2 ms-2 dropdown-toggle" data-toggle="dropdown" data-bs-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
            ...
            {display_icon('it-expand', 'svg', 'icon-expand icon icon-xs icon-white')}
        </button>
        <div class="dropdown-menu">
            <div class="link-list-wrapper">
                <ul class="link-list">
                    ...
                    <li>
                        <a class="list-item" href="#" data-create-relation data-attribute="{$attribute.contentclassattribute_id}" data-class="{$class.identifier}" data-parent="{$browse_object_start_node}">
                            {$class.name|wash()}
                        </a>
                    </li>                    
                    {/foreach}
        ...
        {else}
            <a class="btn btn-sm btn-info ml-1 ms-1" href="#" data-create-relation data-attribute="{$attribute.contentclassattribute_id}" data-class="{$classes[0].identifier}" data-parent="{$browse_object_start_node}">
    ...
    $('[data-create-relation]').on('click', function (e) {
        ...
        },
        {
            ...
            onSuccess: function(data){
                ...
                priority++;
                $.ez('ezjsctemplate::relation_list_row::'+data.content.metadata.id+'::'+container.data('attribute')+'::'+priority+'::?ContentType=json', false, function(content){
                    ...
                    "form": {
                        "buttons": {
                            ...
                            "reset": {
                                ...
                                'id': "reset-" + container.data('attribute') + "button",
