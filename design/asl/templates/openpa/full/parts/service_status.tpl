...
<ul class="d-flex flex-wrap gap-1 my-3">
    <li>
        <div class="chip chip-simple chip-{if $is_active}primary{else}danger{/if} text-button border-{if $is_active}primary{else}danger{/if} bg-light rounded-2" href="#" data-element="service-status">
            <span class="chip-label lh-sm px-2">{$status_tags|implode(', ')|wash()}</span>
        </div>
        {if $is_active|not()}
