...
{/if}
{if is_set($summary)|not()}
    {def $summary = parse_attribute_groups($object, $show_all_attributes)}
{/if}
{if $summary.has_items}
    <div class="row{if $summary.show_index} row-column-menu-left mt-2 pb-40{/if} attribute-list">
        {if $summary.show_index}
        <div class="col-12 col-lg-3 mb-4 border-col">
            <div class="cmp-navscroll sticky-top" aria-labelledby="accordion-title-one" style="top:100px">
                <nav class="navbar it-navscroll-wrapper navbar-expand-lg" data-bs-navscroll="" data-bs-scrollpadding="130">
                                        ...
                                        <div id="collapse-one" class="accordion-collapse collapse show" role="region" aria-labelledby="accordion-title-one">
                                            <div class="accordion-body">
                                                <ul class="link-list" data-element="page-index">
                                                    ...
                                                    {if $item.label}
                                                    <li class="nav-item">
                                                        <a class="py-2 nav-link{if $index|eq(0)} active{/if}" href="#{$item.slug|wash()}">
        ...
        <div class="{if $summary.show_index|not()}col w-100 px-lg-4 pb-lg-4{else}col-12 col-lg-8 offset-lg-1{/if}">
            ...
            {foreach $summary.items as $index => $item}
                <section id="{$item.slug|wash()}" class="it-page-section mb-30{if $summary.show_index} anchor-offset{/if}">
                    {if $item.evidence}<div class="has-bg-grey p-3 my-3">{/if}
                    ...
                    {foreach $item.attributes as $attribute_index => $openpa_attribute}
                        ...
                        {elseif and($openpa_attribute.full.show_label, $item.is_grouped)}
                            ...
                            {else}
                                <h3 class="h6 mt-3 font-sans-serif text-uppercase text-primary subtitle-small">{$openpa_attribute.label|wash()}</h3>
                            {/if}
                        {/if}
