{*
 * $Id: view.tpl 17 2011-06-22 18:50:07Z dpobel $
 * $HeadURL: http://svn.projects.ez.no/ezclasslists/tags/ezclasslists_1_2/ezclasslists/design/admin2/templates/class/view.tpl $
 *}
...
<input class="button toggle-detail" type="button" value="Toggle detail" style="float:right" />
...
<tr>
    <th{if $Attributes.item.is_required} style="background-color:#f00"{/if} colspan="5">
        <input class="button toggle-field-detail" style="font-size: .7em;float:right" type="button" value="Toggle" />
        {$Attributes.number}.&nbsp;{$Attributes.item.nameList[$language_code]|wash}&nbsp;[{$Attributes.item.data_type.information.name|wash}]&nbsp;(id:{$Attributes.item.id})
    </th>
</tr>
...
<script type="text/javascript">
  jQuery(function( $ )//called on document.ready
  {
    $('.toggle-detail').on('click', function (e){
      e.preventDefault()
      $('tr.field-detail').toggle()
    })
    $('.toggle-field-detail').on('click', function (e){
      e.preventDefault()
